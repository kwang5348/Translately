...
public class InfiniteStreamRecognize {
	...
	public static String checkLang(int cid) {
		String result = null;
		switch (cid) {
		case 1:
			result = "en-US";
	...
	public static void infiniteStreamingRecognize(String languageCode, HttpServletRequest servletrequest,
		...
		class MicBuffer implements Runnable {
			...
			public void run() {
				...
				while (targetDataLine.isOpen() && flag) {
					try {
			...
			responseObserver = new ResponseObserver<StreamingRecognizeResponse>() {
				...
				public void onResponse(StreamingRecognizeResponse response) {
					...
					if (result.getIsFinal()) {
						...
						System.out.printf("%s: %s [confidence: %.2f]", convertMillisToDate(correctedTime),
